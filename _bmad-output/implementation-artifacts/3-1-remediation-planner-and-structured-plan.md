...
#### Findings
- [ ] [Review][Patch] Missing read-write MCP URL wiring [`charts/openshift-ai-ops/templates/deployment-backend.yaml:93`] — The backend chart injects `MCP_READONLY_URL` but never `MCP_READWRITE_URL`, while `MCPReadWriteSettings` falls back to `http://mcp-readwrite:8080/mcp` and the Helm service is named `{{ .Release.Name }}-mcp-readwrite`. In a real deployment the planner cannot reach the read-write MCP endpoint.
- [ ] [Review][Patch] Remediation dispatch is not idempotent [`backend/src/pipeline/dispatcher.py:247`] — `dispatch_remediation()` polls every `diagnosed` incident lacking a persisted plan and immediately spawns `_run_remediation_task()` without an in-flight claim. Because `run_remediation_pipeline()` does not transition incident state, repeated polls can start concurrent planners for the same incident, overrun the intended dispatcher concurrency controls, and turn the second insert into a unique-constraint failure instead of a no-op.
- [ ] [Review][Patch] Remediation graph execution is not checkpointed [`backend/src/pipeline/remediation_runner.py:49`] — The diagnosis runner compiles its graph with `get_checkpointer()` and a per-incident `thread_id`, but remediation currently uses `builder.compile()` and `graph.ainvoke(initial_state)` directly. That violates the project rule that LangGraph pipeline state persists to PostgreSQL and leaves remediation runs non-resumable after restarts.
