...
- [x] [Review][Patch] Rebuttal failures still look like a clean skeptic pass [`backend/src/pipeline/remediation_skeptic_validation.py:63`] — When `run_planner_rebuttal()` raises, the loop preserves the existing plan and records `rebuttal_failed` in nested history, but still returns `passed=True` and lets the runner emit `skeptic_validation=validated`. That turns an LLM/tool/schema failure into an apparent successful validation instead of surfacing a degraded/manual-verification outcome. **Fixed**: the runner now emits `skeptic_validation=validated_degraded` when the verdict has `degraded=True`.
- [x] [Review][Patch] Skeptic stage can remain stuck in `validating` after persistence failure [`backend/src/pipeline/remediation_runner.py:126`] — The graph now emits `skeptic_validation=validating` at stage start, but if transactional persistence fails afterward the exception path only emits `remediation_plan=failed`. Subscribers tracking the skeptic stage never receive a terminal failure/aborted event for `skeptic_validation`. **Fixed**: all failure paths now emit terminal events for `skeptic_validation`, `dry_run`, and `remediation_plan`.
...
#### Findings
- [x] [Review][Patch] Rebuttal failures still look like a clean skeptic pass [`backend/src/pipeline/remediation_skeptic_validation.py:63`] — When `run_planner_rebuttal()` raises, the loop preserves the existing plan and records `rebuttal_failed` in nested history, but still returns `passed=True` and lets the runner emit `skeptic_validation=validated`. That turns an LLM/tool/schema failure into an apparent successful validation instead of surfacing a degraded/manual-verification outcome. **Fixed**: the runner now emits `skeptic_validation=validated_degraded` when the verdict has `degraded=True`, allowing subscribers to distinguish degraded from clean validation. New tests verify the degraded flag propagation.
- [x] [Review][Patch] Skeptic stage can remain stuck in `validating` after persistence failure [`backend/src/pipeline/remediation_runner.py:126`] — The graph now emits `skeptic_validation=validating` at stage start, but if transactional persistence fails afterward the exception path only emits `remediation_plan=failed`. Subscribers tracking the skeptic stage never receive a terminal failure/aborted event for `skeptic_validation`. **Fixed**: the outer exception handler now emits terminal `failed` events for `skeptic_validation`, `dry_run`, and `remediation_plan` on all failure paths, ensuring no stage remains stuck in an in-progress state.
