...
#### Findings
- [x] [Review][Patch] Dry-run uses `apply_resource` for non-apply commands [`backend/src/pipeline/dry_run.py:69`] — `run_dry_run_preflight()` forwards every non-empty `step.command` to MCP `apply_resource`, but `RemediationStep` and existing test/plan examples allow generic command strings and non-`apply` actions. Patch/restart/set-resources style steps therefore cannot be validated with the correct MCP operation or manifest payload, so AC #1's server-side validation is not reliably exercised for real plans. **Fixed**: `run_dry_run_preflight()` now gates dry-run execution to `apply` and `create` actions and marks imperative steps as skipped instead of sending them to `apply_resource`.
- [x] [Review][Patch] Any non-exception MCP response is treated as a passed dry-run [`backend/src/pipeline/dry_run.py:77`] — after `client.query()` returns, `_validate_step()` unconditionally records `success=True` and only treats exceptions as failures. Because `ReadWriteMCPClient` collapses tool output to raw text, a server-side validation or admission denial returned as an error payload instead of an exception would still be persisted as a successful dry-run, violating AC #1. **Fixed**: `_validate_step()` now calls `_response_indicates_error()` to inspect MCP response text for error markers (`denied`, `forbidden`, `failed`, `rejected`, etc.) before recording success. New tests verify error-payload detection and clean-response passthrough.
- [x] [Review][Patch] CAS miss drops persisted policy context [`backend/src/pipeline/remediation_runner.py:116`] — `persist_dry_run_result()` and `persist_policy_decision()` only run after `_handle_policy_decision()` applies the state transition. If another actor changes the incident state first, the runner skips persistence and later dry-run/policy SSE completion events, so the approval UI loses the context AC #2 and AC #7 require. **Fixed**: the runner now persists dry-run and policy artifacts before attempting the policy-state CAS, so concurrent state changes no longer discard stored review context.
...
#### Findings
- [x] [Review][Decision] Dry-run steps do not carry the manifest payload required for `apply_resource` [`backend/src/models/remediation.py:35`, `backend/src/pipeline/dry_run.py:95`] — the final implementation now skips imperative actions correctly, but `apply`/`create` steps still only carry a free-form shell command plus a target like `deployment/test`. `apply_resource` is documented and modeled as a YAML-apply operation, yet the plan contract has no field for the manifest body or structured patch payload that the cluster should validate. Without deciding how remediation plans encode the actual resource content, the dry-run cannot verify the object that execution will later send to the cluster, so AC #1 remains under-specified in the shipped contract. **Resolved (Epic 3 retro):** New pipeline stage decided — a manifest-generation phase will be added as a separate story. The phase queries current cluster state, generates one or more YAML manifests into a temp folder, and the execution command applies everything in that folder. This separates LLM planning (natural language) from manifest production (structured YAML).
- [x] [Review][Patch] Dry-run completion SSE is emitted only after policy-state success [`backend/src/pipeline/remediation_runner.py:148`] — `dry_run_node()` emits `dry_run/running` immediately, but the matching `dry_run/complete` event is delayed until after policy artifacts are persisted and `_handle_policy_decision()` returns `True`. If policy-gate persistence fails, audit logging throws, or the state-transition CAS loses the race, subscribers never receive the dry-run completion event even though pre-flight already finished and was already persisted. This leaves the UI stuck in an in-progress dry-run state and violates the story's explicit start/complete event contract. **Fixed**: `dry_run/complete` SSE is now emitted immediately after persistence succeeds, before the policy-gate CAS check, so subscribers always receive the completion event regardless of policy-state outcome.
