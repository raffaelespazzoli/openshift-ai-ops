...
#### Findings
- [ ] [Review][Decision] Dry-run steps do not carry the manifest payload required for `apply_resource` [`backend/src/models/remediation.py:35`, `backend/src/pipeline/dry_run.py:95`] — the final implementation now skips imperative actions correctly, but `apply`/`create` steps still only carry a free-form shell command plus a target like `deployment/test`. `apply_resource` is documented and modeled as a YAML-apply operation, yet the plan contract has no field for the manifest body or structured patch payload that the cluster should validate. Without deciding how remediation plans encode the actual resource content, the dry-run cannot verify the object that execution will later send to the cluster, so AC #1 remains under-specified in the shipped contract.
- [ ] [Review][Patch] Dry-run completion SSE is emitted only after policy-state success [`backend/src/pipeline/remediation_runner.py:148`] — `dry_run_node()` emits `dry_run/running` immediately, but the matching `dry_run/complete` event is delayed until after policy artifacts are persisted and `_handle_policy_decision()` returns `True`. If policy-gate persistence fails, audit logging throws, or the state-transition CAS loses the race, subscribers never receive the dry-run completion event even though pre-flight already finished and was already persisted. This leaves the UI stuck in an in-progress dry-run state and violates the story's explicit start/complete event contract.
